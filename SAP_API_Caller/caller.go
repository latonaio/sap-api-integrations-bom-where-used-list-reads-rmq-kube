...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-bom-where-used-list-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) ByComponent(billOfMaterialComponent, plant string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "BillOfMaterialWhereUsedList"})
	if err != nil {
		c.log.Error(err)
		return
	}
